...
function EnhancedApp() {
  ...
  const [machineCodeMode, setMachineCodeMode] = useState(false);
  ...
  const handleWebSocketMessage = (message) => {
    ...
    switch (message.type) {
      ...
      case 'error':
        setIsLoading(false);
        addMessage('error', `Error: ${message.data.message || message.data}`);
        break;
      case 'status':
        // Handle status updates (e.g., show progress)
        console.log('Status:', message.data.message || message.data);
        break;
      case 'completion':
        // Ensure loading state is cleared on completion
        setIsLoading(false);
        break;
      default:
        console.log('Unknown message type:', message.type);
        // Clear loading state for any unhandled message types
        setIsLoading(false);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await axios.post('/query', {
        ...
        machine_code: machineCodeMode && !confidentialMode,
  ...
  const handleLogin = async (service) => {
    try {
      setIsLoading(true);
      addMessage('system', `🔐 Starting ${service} login process... Check your terminal and browser window.`);
      
      const response = await axios.post(`/login/${service}`);
      console.log(`Login result for ${service}:`, response.data);
      
      if (response.data.success) {
        addMessage('system', `✅ ${service} login successful! You can now use this service.`);
      } else {
        addMessage('system', `❌ ${service} login failed: ${response.data.message}`);
      }
      ...
      console.error(`Failed to initiate login for ${service}:`, error);
      addMessage('system', `❌ Login error for ${service}: ${error.message}`);
      setIsLoading(false);
    }
  };
  ...
  const MessageBubble = ({ message }) => {
    ...
    return (
      <div className={`message ${isUser ? 'user' : isError ? 'error' : isSystem ? 'system' : 'assistant'}`}>
        <div className="message-header">
          {isUser ? <User size={16} /> : isError ? <XCircle size={16} /> : isSystem ? <Shield size={16} /> : <Bot size={16} />}
          ...
          {!confidentialMode && (
            <div className="sidebar-section">
              ...
              <label className="checkbox-option">
                <input
                  type="checkbox"
                  checked={machineCodeMode}
                  onChange={(e) => setMachineCodeMode(e.target.checked)}
                />
                <span>Machine Code Responses</span>
                <small>Structured output for easier processing</small>
              </label>
            </div>
          )}
